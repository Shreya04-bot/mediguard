...
def _to_float(value: str, field_name: str) -> float:
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"vitals.{field_name} must be a number"
            },
        )
...
@router.post(
    "/predict",
    summary="Run disease risk prediction (frontend-facing contract)",
)
async def ai_predict(
    ...
    if payload.diseaseType not in SUPPORTED_DISEASE_TYPES:
        raise HTTPException(
            status_code=422,
            detail={
                "error": "diseaseType is not a supported assessment type"
            },
        )

    bp_match = re.match(
        r"^(\d{2,3})\s*/\s*(\d{2,3})$",
        payload.vitals.bloodPressure.strip(),
    )
    ...
    if not bp_match:
        raise HTTPException(
            ...
            detail={
                "error": (
                    "vitals.bloodPressure must be in the form "
                    "systolic/diastolic"
                )
            },
        )
    ...
    if payload.vitals.gender not in {"male", "female", "other"}:
        raise HTTPException(
            status_code=422,
            detail={
                "error": (
                    "vitals.gender must be male, female, or other"
                )
            },
        )
    ...
    raw = {
        ...
        "fasting_glucose": _to_float(
            payload.vitals.glucose,
            "glucose",
        ),
        "hba1c": _to_float(
            payload.vitals.hba1c,
            "hba1c",
        ),
        "cholesterol_total": _to_float(
            payload.vitals.cholesterolTotal,
            "cholesterolTotal",
        ),
        "cholesterol_hdl": _to_float(
            payload.vitals.cholesterolHdl,
            "cholesterolHdl",
        ),
        "cholesterol_ldl": _to_float(
            payload.vitals.cholesterolLdl,
            "cholesterolLdl",
        ),
        "triglycerides": _to_float(
            payload.vitals.triglycerides,
            "triglycerides",
        ),
    ...
    # -----------------------------------------------------------------------
    # SELECT PRIMARY RISK BASED ON ASSESSMENT FOCUS
    # -----------------------------------------------------------------------

    if payload.diseaseType == "Combined Risk":
        combined_prob = result["combined_risk_score"]
    ...
    recommendations = _build_recommendations(
        risk_level,
        selected.get("shap_features", []),
        payload,
    )

    preventive_ayurveda = _build_ayurveda_tips(
        result["diabetes"]["risk_level"],
        result["cardiovascular"]["risk_level"],
    )
    ...
    if current_user:
        try:
            ...
            logger.warning(
                "Could not persist prediction to DB",
                exc_info=True,
            )
    ...
    return {
        ...
        "diabetesRisk": {
            "probability": round(
                result["diabetes"]["probability"] * 100,
                1,
            ),
            "riskLevel": _map_risk_level(
                result["diabetes"]["risk_level"]
            ),
        },
        ...
        "cardiovascularRisk": {
            "probability": round(
                result["cardiovascular"]["probability"] * 100,
                1,
            ),
            "riskLevel": _map_risk_level(
                result["cardiovascular"]["risk_level"]
        ...
        "hypertensionRisk": {
            "probability": round(
                result["hypertension"]["probability"] * 100,
                1,
            ),
            "riskLevel": _map_risk_level(
                result["hypertension"]["risk_level"]
            ),
        },

        "combinedRiskScore": round(
            result["combined_risk_score"] * 100,
            1,
        ),
...
def _build_recommendations(
    ...
    top contributing factors — not a generic canned list.
    """
    recs: List[str] = []
    ...
    factor_hints = {
        "fasting_glucose": (
            "Schedule a follow-up HbA1c and fasting glucose "
            "panel with your doctor."
        ),
        "hba1c": (
            "Discuss glycaemic control options (diet, medication) "
            "with your doctor given your HbA1c reading."
        ),
        "cholesterol_ldl": (
            "Consider a lipid-lowering consultation — your LDL "
            "is a leading contributor to this risk."
        ),
        "cholesterol_total": (
            "A follow-up lipid panel is advisable given your "
            "total cholesterol reading."
        ),
        "triglycerides": (
            "Reducing refined carbohydrates and alcohol can help "
            "lower triglycerides."
        ),
        "blood_pressure_systolic": (
            "Monitor blood pressure regularly; consider a "
        ...
        "bmi": (
            "A structured weight-management plan may meaningfully "
            "reduce this risk."
        ),
        "smoking": (
            "Smoking cessation is the single highest-impact "
    ...
    if risk_level in ("high", "critical"):
        recs.insert(
            0,
            "Book an appointment with a physician promptly "
            "to review these results.",
        )

    elif not recs:
        recs.append(
            "Maintain current lifestyle habits and repeat "
            "this assessment periodically."
        )

    recs.append(
        "This is decision support only — always confirm "
...
def _build_ayurveda_tips(
    ...
    try:
        ...
        logger.warning(
            "Ayurveda herb recommendation unavailable",
            exc_info=True,
        )
        return []
...
def _analyze_pdf_bytes(
    ...
    but for text-based PDFs.
    """
    from features.ocr_autofill import (
        _extract_with_regex,
        CLINICAL_REFERENCE,
    )
    ...
    critical_flags = [
        (
            f"{fld}: {data['value']} "
            f"{CLINICAL_REFERENCE.get(fld, {}).get('unit', '')} "
...
@router.post(
    "/ocr",
    summary="Parse a lab report image/PDF (frontend-facing contract)",
)
async def ai_ocr(
    ...
    raw_b64 = (
        payload.imageBase64.split(",", 1)[-1]
        if "," in payload.imageBase64
        else payload.imageBase64
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail={
                "error": "OCR engine temporarily unavailable"
            },
        ) from exc
    ...
    if not fields:
        raise HTTPException(
            ...
            detail={
                "error": (
                    "Could not extract legible biomarker "
    ...
    biomarkers = [
        {
            ...
            "range": _reference_range_str(
                field
            ),
            "status": _biomarker_status(
                data.get(
                    "clinical_status",
                    {},
                ).get(
    ...
    critical_flags = analysis.get(
        "critical_flags",
        [],
    )

    ai_summary = (
        f"Extracted {len(biomarkers)} biomarker(s) "
        "from this report. "
        + (
            f"{len(critical_flags)} value(s) flagged outside "
            "WHO/ADA/ACC reference ranges."
            if critical_flags
            else (
                "All extracted values are within normal "
                "reference ranges."
            )
        )
    )
    ...
    actionable_plan = (
        [
            f"Discuss with your doctor: {flag}"
    ...
    if current_user:
        try:
            ...
            logger.warning(
                "Could not persist OCR report to DB",
                exc_info=True,
            )
...
_ROLE_SYSTEM_PROMPTS = {
    "patient": (
        "You are MediGuard AI's patient-facing health assistant. "
        "Explain things in plain, reassuring language a non-clinician "
        "can follow. You can explain predictions, lab reports, general "
        "risk factors, and medications in general terms, and suggest "
        "sensible follow-up actions. You are not a substitute for a "
        "doctor: for diagnosis, dosing, or urgent symptoms, always "
        "recommend the person consult their physician or emergency "
        "services. Never fabricate a specific diagnosis or prescribe "
        "a specific drug/dose."
    ),

    "doctor": (
        "You are MediGuard AI's clinical assistant for a doctor. "
        "You may use clinical terminology and reference guideline-style "
        "reasoning (ICMR/ADA/ACC/JNC-8 style thresholds) to help "
        "interpret risk scores, lab values, and differential "
        "considerations. You are decision support, not the final word "
        "— flag clinical judgement calls back to the physician rather "
        "than asserting them."
    ),

    "admin": (
        "You are MediGuard AI's platform assistant for an administrator. "
        "Help with platform usage, workflow, and general product "
        "questions. You do not have access to specific patient records "
        "through this chat."
    ),
}
...
def _check_rate_limit(key: str) -> None:
    ...
    while (
        window
        and now - window[0] > _RATE_LIMIT_WINDOW_SECONDS
    ):
        window.popleft()
...
@router.post(
    "/chat",
    summary="Conversational AI assistant (frontend-facing contract)",
)
async def ai_chat(
    ...
    role = (
        payload.userRole
        if payload.userRole in _ROLE_SYSTEM_PROMPTS
        else "patient"
    )

    system_prompt = _ROLE_SYSTEM_PROMPTS[role]

    # RAG grounding: best-effort.
    context = None
    ...
    try:
        ...
        if docs:
            context = "\n\n".join(
                (
                    f"[{d.metadata.get('source', 'guideline')}] "
                    f"{d.page_content}"
                )
                for d in docs[:3]
            )
            ...
            system_prompt += (
                ...
                f"(cite the source tag if you use these):\n{context}"
            )

    except Exception:
        logger.info(
            "RAG retrieval unavailable for this chat turn; "
