  ...
  return (
    <div className="space-y-6">
      <DashboardHero
        user={user}
        role="doctor"
        title={`Good morning, Dr. ${user?.name?.split(" ").slice(1).join(" ") || user?.name || ""}`}
        subtitle={`You have ${patients.length} linked patient${patients.length === 1 ? "" : "s"}${
          pendingRequests.length > 0
            ? ` and ${pendingRequests.length} pending connection request${pendingRequests.length === 1 ? "" : "s"}.`
            : "."
        }`}
        primaryLabel="View Patients"
        primaryHref="/dashboard/doctor/patients"
        secondaryLabel="Open Clinical Assistant"
        secondaryHref="/dashboard/doctor/assistant"
      />
